	...
	for(i=0;i<jogo->numero_barcos;i++){
		...
		if(!n_vira){
			if(jogo->barcos[i]->orientacao){ //muda pra vertical
				for(j=1;j<jogo->barcos[i]->tam_barco;j++){
					jogo->mapa[jogo->barcos[i]->linha-j][jogo->barcos[i]->coluna] = jogo->mapa[jogo->barcos[i]->linha][jogo->barcos[i]->coluna+j];
					jogo->mapa[jogo->barcos[i]->linha][jogo->barcos[i]->coluna+j]='O';
					jogo->barcos[i]->orientacao=0;
				}
			}
			else{//muda pra horizontal
				for(j=1;j<jogo->barcos[i]->tam_barco;j++){
					jogo->mapa[jogo->barcos[i]->linha][jogo->barcos[i]->coluna+j] = jogo->mapa[jogo->barcos[i]->linha-j][jogo->barcos[i]->coluna];
					jogo->mapa[jogo->barcos[i]->linha-j][jogo->barcos[i]->coluna]='O';
	...
	for (x = 0; x < jogo->numero_barcos; x++){
		for (i = 0; i < jogo->barcos[x]->tam_barco; i++){
			if (jogo->barcos[x]->orientacao){
				...
				jogo->mapa[jogo->barcos[x]->linha-i][jogo->barcos[x]->coluna] = 'X';//mudei pra linha-i, em vez de linha+i, ja que o barco cresce pra cima
			}
		}
	}
}
